...
####this section is added to direct user to the profile page after login
from django.views.generic import ListView
from braces.views import LoginRequiredMixin
from jokes.models import JokeUser

class ProfileView(LoginRequiredMixin, TemplateView):
    template_name = "userprofiles/home." + app_settings.TEMPLATE_EXTENSION

    def get_context_data(self, **kwargs):
        context = super(ProfileView, self).get_context_data(**kwargs)
        context['username'] = self.request.user
        context['profile_self_jokes'] = JokeUser.objects.filter(user=self.request.user).order_by('-time_created')
        return context
##########################################################################
